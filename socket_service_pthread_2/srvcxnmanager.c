...
void *threadProcess(void *ptr) {
    ...
    while ((len = read(connection->sockfd, buffer_in, BUFFERSIZE)) > 0) {
        ...
        if (strncmp(buffer_in, "bye", 3) == 0) {                        //Bye pour quitter la connexion
            break;
        }
#if DEBUG //Debug
        printf("DEBUG-----------------------------------------------------------\n");
        ...
        if (buffer_in[0] == '@') {                                      //Si message commence par @, le message est envoyé a tout le monde
            for (int i = 0; i < MAXSIMULTANEOUSCLIENTS; i++) {
                if (connections[i] != NULL) {
                    write(connections[i]->sockfd, buffer_out, strlen(buffer_out));
                }
            }
        } else if (buffer_in[0] == '#') {                               //Si message commence par #?, le message est envoyé au client ciblé 
            int client = 0;
...
int create_server_socket() {
    ...
    int port = 7799;                //Ecoute sur ce port
    ...
    return sockfd;              //Numero de flux (on peut ecrire dedans)
}
